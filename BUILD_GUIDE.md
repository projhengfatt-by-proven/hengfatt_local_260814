...
**2026-08-13 — Reconciled the project's doc set (`AGENTS.md`, `.ai/*.md`, `ONBOARDING_CHECKLIST.md`); found and fixed one real conflict**

- Discovered another Claude Code session had, in parallel, set up a centralized cross-project AI-skill system (`AGENTS.md`, `.ai/project-context.md`, `.ai/project-status.md`, `.ai/skill-feedback.md`, `.ai/skills-manifest.yaml`, per the generic template `MASTER_05_CREATE_DYNAMIC_PROJECT_AGENT.md`), and a third session had begun wiring n8n live for real (`ONBOARDING_CHECKLIST.md`) and extended this guide with **Part E** (§17-19, the dual-execution pattern formalized as a reusable template) and **Part F** (§20-21, onboarding/offboarding gap tracking) — all since I'd last read the whole file. Went through plan mode to read every `.md` file directly and check for contradictions before touching anything, given multiple sessions editing concurrently with no git.
- Found one real, unflagged conflict: **§13 ("Step-by-step: deploying Phase 1") still instructed standing up n8n on Railway**, but `ONBOARDING_CHECKLIST.md` (same day, later) documents that n8n is actually self-hosted inside a VPN behind a reverse proxy — completely different infrastructure, already built. §13 was never updated to say so, and the same stale "Railway" framing had also spread into `.ai/project-context.md` and `.ai/project-status.md`. Fixed all three with an explicit "Superseded 2026-08-13" note pointing at `ONBOARDING_CHECKLIST.md` Section A as the real current checklist, leaving the historical Railway walkthrough intact below the note rather than deleting it (matches this doc's own convention of preserving history).
- Everything else cross-checked consistent across every file that mentions it: n8n's overall not-live status, the 3 pending migrations, the `aria-whatsapp` gap, the dual-execution rule, the no-git fact.
- Recommended **against** merging everything into a single `AGENTS.md`, per the user's question — the files are a deliberate two-tier system (terse agent-bootstrap vs. detailed narrative vs. concise machine-snapshot vs. literal QA checklist), not redundant copies; merging would degrade all four roles. Full reasoning in the approved plan file.
- Next: confirm with the onboarding-flow session before setting the Supabase-side n8n secrets I have queued, since their VPN infrastructure work may still be in flight.

**2026-08-14 — First git history for this project: initialized, pushed to GitHub, prepping for Vercel**

- User wants to move hosting to Vercel (better fit for this app's dynamic/SPA nature than Lovable's own hosting). Vercel needs a git remote to deploy from, and this repo had never had one — `.env` also wasn't in `.gitignore` yet, so this had to be done carefully, not just `git init && push`.
- **Multi-account care**: the GitHub account for this project is different from the one already cached on this laptop. Rather than risk git silently pushing under the wrong identity, generated a **dedicated SSH keypair** (`~/.ssh/id_ed25519_hengfatt`, never touching the existing default key) and a matching `~/.ssh/config` `Host github-hengfatt` alias with `IdentitiesOnly yes` — this repo's remote uses that alias exclusively, so it can never touch or be confused with the other account's credentials. User added the public key to the correct account via GitHub's Settings → SSH and GPG keys themselves; I verified with `ssh -T git@github-hengfatt` before touching git.
- Fixed `.gitignore` **before** the first commit (verified with `git status --short` that nothing secret was staged, not just assumed): added `.env`/`.env.*` (client only had the public Supabase anon key, but still shouldn't be committed — real secrets get set directly in Vercel's/Supabase's dashboards, never in a committed file) and `.claude/` (local AI-tool state, e.g. `scheduled_tasks.lock` — not project source).
- Repo: **`projhengfatt-by-proven/hengfatt_local_260814`** (private), pushed to `main`. Local commit identity (`git config user.name/email`) set repo-scoped, not global, so it doesn't affect the user's other projects.
- Recorded the new deployment state in `.ai/project-context.md` (Deployment section) and `.ai/project-status.md` (closed the "git never initialized" Technical Debt item) so this doesn't drift stale like the Railway/VPN item did.
