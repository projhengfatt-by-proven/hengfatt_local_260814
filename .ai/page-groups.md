...
| Market Insights | Manage public insight cards, detail copy, links, and ordering. | Admin starts with the list view, opens an insight for detail, and only opens the form when creating or editing. Featured and display order controls decide which insight is shown first. |
...
- Market insights now behave like real articles: the admin starts from the list, can open a detail page, and manages the card excerpt, body copy, cover image, publish state, featured status, and display order from the form/list.
